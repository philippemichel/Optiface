...
Un tableau présentant les données démographiques de la population sera présenté

# Objectif principal

*Score d’attractivité de 1 à 5 (respectivement de très peu esthétique à très esthétique). Les images seront évaluées par 4 évaluateurs indépendants (2 experts : chirurgien de la face et 2 non-experts) en fonction de la focale utilisée : 50 mm, 85mm, 105mm, 135mm et 200mm. Les participants seront classés selon le Phototype Fitzpatrick : un groupe I-II, un groupe III-IV et un groupe V-V*

On vérifiera tout d'abord la concordance des évaluations des experts par un test de Kappa-Cohen (package `psych` [@psych]).

Le critère de tri est une classification en cinq groupes (1 à 5) qui seront considérés comme des notes (données numériques). Les résultats seront tout d'abord étudiés globalement par une comparaison des moyennes des notes obtenues par les différentes focales (test de \gls{kruskal-wallis}). Si le test est significatif, une \gls{analyse post-hoc} sera réalisée pour identifier des différences éventuelles entre les groupes de focales pris deux à deux (test de \gls{tukey} -- packages `emmeans` [@emmeans]).

On recherchera ensuite un à un les critères pouvant influer sur ce résultat c'est à dire le phototype de Fitzpatrick et de la qualité des évaluateurs (experts vs non-expert) par des analyses en régression linéaire. Si un test est significatif, une a\gls{analyse post-hoc} sera réalisée pour identifier des différences éventuelles entre les groupes de focales pris deux à deux (test de v\gls{tukey} -- packages `emmeans` [@emmeans]). On présentera ensuite l'influence éventuelle d'autres facteurs (sexe, âge\dots) qui pourraient influer sur le résultat.  

Une analyse globale en régression linéaire reprenant tous ces critères à savoir la qualité de l"évaluateur le phototype de Fitzpatrick  & les les critères autre ayant une réponse significative avec un seuil de la p.value \< 0,2 lors de l'étape précédente. Une recherche du meilleur modèle sera réalisée par un step-by-step descendant évaluée par le critère d’information d’Akaike. Cette étape sera réalisée après imputation des données manquantes. Une \gls{analyse post-hoc} sera réalisées par la procédure décrite plus haut si nécessaire.
