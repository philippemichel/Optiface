...
Le critère de tri est une classification en cinq groupes (1 à 5) qui seront considérés comme des notes (données numériques). Les résultats seront tout d'abord étudiés globalement par une comparaison des moyennes des notes obtenues par les différentes focales (test de \gls{kruskal}). Si le test est significatif, une \gls{analyse post-hoc} sera réalisée pour identifier des différences éventuelles entre les groupes de focales pris deux à deux (test de \gls{tukey} -- packages `emmeans` [@emmeans]).
